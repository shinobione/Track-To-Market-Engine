  ...
  return <div className="app-shell">
    <aside className="sidebar">
      <div className="sidebar-scroll">
        ...
        <div className="local-badge">LOCAL-FIRST · FREE</div>
        ...
        <div className="stats"><div><span>Actions session</span><b>{stats.actions}</b></div><div><span>API / crédits</span><b>0 € / 0</b></div><p>Texte, artwork, formats, teaser et ZIP restent dans le navigateur.</p></div>
      </div>
      <div className="sidebar-bottom">{error && <div className="error-banner">{error}</div>}<Button onClick={generate}>✦ Générer le Release Pack</Button><small>v0.1.0 · GitHub Pages / Studio-ready</small></div>
    </aside>

    <main className="main-content">{!pack ? <div className="empty-state"><div className="empty-symbol">↗</div><span>TRACK → MARKET</span><h1>From finished track<br/><em>to release assets.</em></h1><p>Un moteur local pour préparer artwork, copy, tags, formats et teaser sans abonnement ni clé API.</p><div className="empty-pills"><span>SoundCloud</span><span>Social</span><span>Artwork</span><span>ZIP</span></div></div> : <OutputDisplay pack={pack} params={params} onPackChange={setPack} onRegenerate={generate} onTrackAction={() => setStats(previous => ({ actions: previous.actions + 1 }))}/>}</main>
  </div>;
}

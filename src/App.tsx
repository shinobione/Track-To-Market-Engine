...
const DRAFT_KEY = 'shinobiwan:track-to-market:draft:v0.1.1';
  ...
  return <div className="app-shell">
    <aside className="sidebar">
      <div className="sidebar-scroll">
        ...
        <div className="local-badge">AI-FIRST · QUALITY MODE</div>
        ...
        <div className="stats"><div><span>Actions session</span><b>{stats.actions}</b></div><div><span>Artwork</span><b>FLUX.2 dev</b></div><p>Artwork IA pur + titre/logo composés localement avec précision. Brouillon sauvegardé automatiquement dans ce navigateur.</p></div>
      </div>
      <div className="sidebar-bottom">{error && <div className="error-banner">{error}</div>}<Button onClick={generate}>✦ Générer / recalculer le Release Pack</Button><small>v0.1.1 · draft autosave · AI quality</small></div>
    </aside>

    <main className="main-content">{!pack ? <div className="empty-state"><div className="empty-symbol">↗</div><span>TRACK → MARKET</span><h1>From finished track<br/><em>to release assets.</em></h1><p>Release copy + vraie génération de covers IA, formats cohérents, teaser et export ZIP. Tes inputs restent sauvegardés localement pendant les itérations.</p><div className="empty-pills"><span>SoundCloud</span><span>Social</span><span>FLUX.2 dev</span><span>ZIP</span></div></div> : <OutputDisplay pack={pack} params={params} onPackChange={setPack} onRegenerate={generate} onTrackAction={() => setStats(previous => ({ actions: previous.actions + 1 }))}/>}</main>
  </div>;
}
